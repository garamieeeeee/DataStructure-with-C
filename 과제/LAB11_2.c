...
int ReadFile(char* fileName, BinTree* tree) {
	...
	while (fscanf(file, " %c", &data) != EOF) {
		if (data != 'N') tree->nodes[i] = CreateNode(data);
		i++;
		count++;
	}
	...
	for (int i = 1; i <= count; i++) {
		if (tree->nodes[i].data != 0) {
			if ((2 * i <= count) && (tree->nodes[2 * i].data != 0)) tree->nodes[i].leftChild = &(tree->nodes[2 * i]);
			if ((2 * i + 1 <= count) && (tree->nodes[2 * i + 1].data != 0)) tree->nodes[i].rightChild = &(tree->nodes[2 * i + 1]);
		}
	}
...
int main(int argc, char* argv[]) {
	...
	/*for (int i = 1; i <= MAX_NODE_COUNT; i++) {
		printf("[%d] %c ", i, tree.nodes[i].data);
	}
	printf("\n");*/
	...
	/*for (int i = 1; i <= n; i++) {
		if (tree.nodes[i].data != 0) {
			...
			if (tree.nodes[i].leftChild != NULL)
				printf("L: %c, ", tree.nodes[i].leftChild->data);
			if (tree.nodes[i].rightChild != NULL)
				printf("R: %c, ", tree.nodes[i].rightChild->data);
		}
	}
	printf("\n");*/
