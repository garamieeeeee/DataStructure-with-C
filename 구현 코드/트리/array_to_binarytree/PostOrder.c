...
//PostOrder tree traversal의 recursive version을 이용하여 구성
//즉 L->R->V 순으로 노드를 생성하여 재귀적으로 트리를 구성함
...
BinTreeNode* BuildTree(int index) { //index: 루트노드의 노드번호
	...
	//(L->R->V) Postorder tree traversal의 recursive version 이용
	...
	//array[]의 (2*index)번의 데이터를 가지고 다시 BuildTree() 순환 호출, 왼쪽 자식 생성
	BinTreeNode* leftChild = BuildTree(2 * index);
	//array[]의 (2*index+1)번의 데이터를 가지고 다시 BuildTree() 순환 호출, 오른쪽 자식 생성
	BinTreeNode* rightChild = BuildTree(2 * index + 1);
	//array[]의 index번의 데이터를 가지고 부모노드 parent 생성
	//leftChild, rightChild 달기
	BinTreeNode* parent = CreateNode(array[index]);
	parent->leftChild = leftChild;
	parent->rightChild = rightChild;

	return parent; //부모 노드를 반환 (종료 조건2)
}
