//min heap의 삽입,삭제 연산 & 최소힙을 이용해 자료를 내림차순으로 정렬하는 연산
...
void Insert_minHeap(HeapType* h, ElementType newNode) { //최소 힙 h에 새로운 노드 new를 삽입 
	...
	//도중에 (i번)new의 key가 (i/2번) 부모노드의 키보다 크거나 같아지면 더 이상 올라가지 않고 멈춘다. 그자리에 new를 삽입한다.
	// i==1, 즉 루트노드 위치까지 오면 비교를 멈추고 그 자리에 new를 삽입한다.
	while ((i != 1) && (newNode.key < h->heap[i / 2].key)) {
		h->heap[i] = h->heap[i / 2]; //만약 i번 키가 부모노드인 i/2번 노드의 키보다 작다면, i/2번 노드가 밑으로 내려와야한다. 부모노드를 자식노드의 위치로 끌어내린다.
...
ElementType Delete_minHeap(HeapType* h) { //루트노드(=최솟값)을 삭제하여 그 값을 반환
	...
	while (child <= h->heapSize) { //child가 heapSize보다 커지면, 즉 트리의 마지막 노드까지 비교했는데 temp가 더 크다면 그 위치에 temp가 들어가야한다.
		//현재 노드의 자식노드(왼쪽/오른쪽) 중에서 더 작은 값을 찾는다. 둘 중에 더 작은 노드와 temp가 교환된다.
		if ((child < h->heapSize) && h->heap[child].key > h->heap[child + 1].key) { //만약 오른쪽 자식이 더 작다면
			child++; //오른쪽 노드와 temp를 비교해야 함. 만약 왼쪽 자식이 더 작다면 ++하지 않는다
		}
		if (temp.key <= h->heap[child].key) break; //temp의 키가 자식노드보다 작아졌다면 알맞은 위치를 찾은 것이므로 이제 더 내려갈 필요 없다.
		//temp의 키가 자식노드보다 큰 것이므로, 자식노드가 위로 올라와야 함
...
void HeapSort(ElementType e[], int n) { //최소 히프를 이용해 내림차순으로 정렬하는 함수(n: 배열의 크기)
